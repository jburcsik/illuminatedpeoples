...
if (file_exists($path)){
	...
	// Download all the images and rewrite the references in the array to their local location
...
if (file_exists($backup)){
	...
	for ($i = 0; $i < count($results[0]->statuses); $i++){
		...
		// Check if the image url is already illuminated peoples.
		// if ($results[0]->statuses[$i]->entities[0]->media[0]->media_url[0] ... HAS illuminatedPeoples in it){
		
		//	for ($j = 0; $j < count($current[0]->statuses); $j++){
				
			echo $results[0]->statuses[$i]->entities->media->media_url;
			echo '* checking for the urls';
			
			// Download to path

			// Udpate the reference we just used to find the image	
					break;
